...
object Main extends App {
 ...
 val executables: List[Task[Either[ErrorPayload, Unit]]] =
   List(bankInfo, deedOfAssignment, performance, NoW)
     .map(data => TemplateFactory.renderTemplate(data))

  Task.gatherUnordered(executables).foreach(exs =>
    exs.map {
      case Right(_) => ()
